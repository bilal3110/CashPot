...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", function (event) {
    // Don't trigger on clicks to buttons or specific elements
    if (event.target.tagName === "BUTTON" || event.target.classList.contains("no-fullscreen")) {
      return;
    }

    // Only trigger fullscreen on click when not already in fullscreen
    if (!isFullScreen && (isAndroid || isWindows)) {
